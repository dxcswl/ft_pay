...
class Weixin {
     ...
     *
     */
    public function getWeixin($config, $money, $pay_id, $trade_type = 'APP') {
        $result_code = $this->getWeixinUnifiedOrder($config, $money, $pay_id,$trade_type);
        ...
        if(isset($result['appid']) and isset($result['nonce_str']) and isset($result['mch_id']) and isset($result['prepay_id']) and isset($result['sign'])) {
            ...
            if($trade_type == 'APP'){
                ...
                $data['signType'] = $config['sign_type']?:'MD5';
            }
            ...
            return ['code' => 0, 'msg' => $result['msg'], 'data' => []];
        }
    }

    private function getWeixinUnifiedOrder($config, $money, $pay_id, $trade_type = 'APP') {
        $ret = ['code' => 0, 'msg' => '未知', 'data' => []];
        //检测必填参数
        if(isset($pay_id) && !$pay_id) {
            return ['code' => 0, 'msg' => '缺少统一支付接口必填参数out_trade_no', 'data' => []];
        } else if(isset($config['body']) && !$config['body']) {
            return ['code' => 0, 'msg' => '缺少统一支付接口必填参数body', 'data' => []];
        } else if(isset($config['appid']) && !$config['appid']) {
            return ['code' => 0, 'msg' => '缺少统一支付接口必填参数appid', 'data' => []];
        } else if(isset($config['key']) && !$config['key']) {
            return ['code' => 0, 'msg' => '缺少统一支付接口必填参数key', 'data' => []];
        } else if(isset($config['mchid']) && !$config['mchid']) {
            return ['code' => 0, 'msg' => '缺少统一支付接口必填参数mchid', 'data' => []];
        } else if(!$money) {
            return ['code' => 0, 'msg' => '缺少统一支付接口必填参数total_fee', 'data' => []];
        } elseif($trade_type == "JSAPI" && isset($config['openid']) && !$config['openid']) {
            return ['code' => 0, 'msg' => '统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数', 'data' => []];
        } elseif($trade_type == "NATIVE" && isset($config['product_id']) && !$config['product_id']) {
            ...
            if($xml == false){
                ...
                $response = $this->postXmlCurl($xml, $url, false, 6,$config);
                ...
                if($result == false){
                    ...
                    $response = $this->postXmlCurl($xml, $url, false, 6,$config);
                    //验证签名
                    $check_sign = $this->checkSign($response,$config['key']);
            ...
            //$this->reportCostTime($url, $this->getMillisecond(), $result, $config);//上报请求花费时间
        }
        return $ret;
    }
     ...
     * @throws \Exception
     */
    private function postXmlCurl($xml, $url, $useCert = false, $second = 30,$config=[]) {
        $ch = curl_init();
        //设置超时
        curl_setopt($ch, CURLOPT_TIMEOUT, $second);

        //如果有配置代理这里就设置代理
        if(isset($config['CURL_PROXY_HOST']) && $config['CURL_PROXY_HOST'] != "0.0.0.0" && isset($config['CURL_PROXY_PORT']) && $config['CURL_PROXY_PORT'] != 0) {
     ...
     * @param array $data
     */
    private function reportCostTime($url, $startTimeStamp, $data, $config) {
        //如果不需要上报数据
    ...
    public function getNotify($config)
    {
        ...
        try {
            //验证签名
            $check_sign = $this->checkSign($xml,$config['key']);
            if($check_sign['code'] == 1){
                $values = $check_sign['data'];
                //查询订单，判断订单真实性
                $orderQuery = $this->orderQuery($values["transaction_id"],$config);
                $orderQuery['data']['total_fee'] = isset($values['total_fee'])?$values['total_fee']:0;
                $orderQuery['data']['transaction_id'] = isset($values['transaction_id'])?$values['transaction_id']:0;
                $orderQuery['data']['out_trade_no'] = isset($values['out_trade_no'])?$values['out_trade_no']:0;
                $orderQuery['data']['time_end'] = isset($values['time_end'])?$values['time_end']:0;
                $orderQuery['data']['openid'] = isset($values['openid'])?$values['openid']:0;
                $orderQuery['data']['trade_type'] = isset($values['trade_type'])?$values['trade_type']:0;
                return ['code' => 1, 'msg' => '验证成功', 'data' => ['request'=>input('param.'),'query'=>$orderQuery]];
            }else{
                return ['code' => 0, 'msg' => 'code 错误', 'data' => ['request'=>input('param.'),'query'=>$check_sign]];
            }
    ...
    public function checkSign($xml,$key){
        ...
        if($values['sign'] != $this->makeSign($values, $key)){
     ...
     * @return 成功时返回，其他抛异常
     */
    public function orderQuery($transaction_id, $config)
    {
     ...
     *
     */
    public function setWeixinRefund($data, $wxchat = [])
    {
        //判断有没有CA证书及支付信息
...
//            $wxchat['api_ca'] = APP_PATH .$this->cacab['api_ca'];
        ...
        $webdata = array(
            ...
            'spbill_create_ip' => $this->refundGetIp(),
        );
        ...
        $wget = $this->refundArrayXml($webdata);
        $res = $this->refundHttpPost('https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers', $wget, $wxchat);
        if (!$res) {
     ...
     */
    public function refundArrayXml($arr, $level = 1)
    {
        $s = $level == 1 ? "<xml>" : '';
        foreach ($arr as $tagname => $value) {
            ...
            if (!is_array($value)) {
                ...
                $s .= "<{$tagname}>" . $this->refundArrayXml($value, $level + 1) . "</{$tagname}>";
            }
        }
    ...
    public function refundHttpPost($url, $param, $wxchat)
    {
        $oCurl = curl_init();
